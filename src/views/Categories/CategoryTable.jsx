...
export default function CategoryTable() {
    ...
    const getCategories = (page = 1) => {
        setLoading(true);
        axiosClient.get(`/categories?page=${page}`)
            .then(({ data }) => {
                setLoading(false);
    ...
    const onDelete = (category) => {
        ...
        axiosClient.delete(`/categories/${category.id}`)
            .then(() => {
                getCategories(meta.current_page);
            })
            .catch(err => {
                                ...
                                <tr key={category.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            <button onClick={() => onDelete(category)} className="text-red-600 hover:text-red-900 p-1 rounded hover:bg-red-50" title="Delete">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                ...
                {!loading && meta.links && (
                    <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200 bg-gray-50">
                        <div className="flex-1 flex justify-between sm:hidden">
                            <button
                                onClick={() => getCategories(meta.current_page - 1)}
                            ...
                            <div>
                                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    {meta.links.map((link, index) => (
                                        <button
                                            key={index}
                                            onClick={() => link.url && getCategories(link.label.includes('Previous') ? meta.current_page - 1 : (link.label.includes('Next') ? meta.current_page + 1 : link.label))}
